  ...
  draw() {
    ...
    for (let i = 1; i <= this.length; i++) {
      ...
      if (this.turns.length > 0) {
        console.log('before turning');
        console.log([bodySegmentX, bodySegmentY]);
        console.log('after turning')
        console.log(this.turns[0])

        if (['ArrowUp', 'ArrowDown'].includes(this.currentDirection)) {
            bodySegmentX = this.turns[0][0];
        } else if (['ArrowLeft', 'ArrowRight'].includes(this.currentDirection)) {
            bodySegmentY = this.turns[0][1];
        }
