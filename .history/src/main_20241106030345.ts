...
const checkCollision = (coordinates: number[]) => {
    snake.paths.forEach(path => {
...
class Snake {
  ...
  checkCollision(headCoordinates: [number, number]) {
    const collided =
      this.paths.findIndex(
        (path) =>
          path[0] === headCoordinates[0] && path[1] === headCoordinates[1]
      ) === -1;
