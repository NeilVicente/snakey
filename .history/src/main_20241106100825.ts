...
class Food {
  x: number = Math.floor(Math.random() * 40) * 10;
  y: number = Math.floor(Math.random() * 40) * 10;
...
class Snake {
  x: number = Math.abs(Math.floor(Math.random() * 40)) * 10;
  y: number = Math.abs(Math.floor(Math.random() * 40)) * 10;
  length: number = 10;
  ...
  update() {
    ...
    if (this.currentDirection === "ArrowRight") {
      if (headCoordinates[0] > game.width) {
        headCoordinates[0] = 0;
      } else if (headCoordinates[0] < 0) {
        headCoordinates[0] = game.width;
      }
...
function gameLoop() {
  ...
  ctx!.clearRect(0, 0, game.width, canvas.height);
  counter = 0;
